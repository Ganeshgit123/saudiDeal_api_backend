...
export default class AuthController {
  ...
  public async sendOtp({ request }: HttpContextContract) {
    ...
    try {
      ...
      console.log('SMS response:', response && response.status)
      if (response && response.status == 201) otpResult = true
      else otpResult = false
    } catch (err) {
      console.log('SMS send error:', err && err.response ? err.response.data || err.response.statusText : err)
      otpResult = false
    }
  ...
  public async create({ request }: HttpContextContract) {
    ...
    const data = {
      ...
      isNewUser: 0,
      userType: userType,
      otp: mobileNumber == 123456789 ? 1234 : otp
    }
    ...
    const config: AxiosRequestConfig = {
      ...
      data: {
        "recipients": [
          // Ensure recipient includes country code
          `${countryCode}${mobileNumber}`
        ],
        "body": messageText,
        "sender": "SaudiDeal"
